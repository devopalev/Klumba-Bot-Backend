...
def send_message_user(context: CallbackContext):
    from source.event_bot.core import bot as bot_event
    query_components = context.job.context

    bitrix_id = Utils.prepare_external_field(query_components, "bitrix_id", escape_md=False)
    if bitrix_id:
        bitrix_id = bitrix_id.lstrip("user_")
    text = Utils.prepare_external_field(query_components, "text", escape_md=False)

    for _, user_data in context.dispatcher.user_data.items():
        user = user_data.get(cfg.USER_PERSISTENT_KEY)
        if user.bitrix_user_id == bitrix_id:
            try:
                bot_event.send_message(user.tg_user.id, text, parse_mode=ParseMode.HTML, disable_web_page_preview=True)
            except Exception as err:
                logging.warning(f"Не удалось отправить уведомление пользователю ({user.__dict__}): {err}")
            break
    else:
        logging.warning(f"Пользователь (b24 id {bitrix_id}) не авторизирован")
