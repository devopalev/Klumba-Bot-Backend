...
def view_deal(update, context, user):
    ...
    user.deal.deal_id = deal_id  # save for future use commands based on deal view
...
def view_order_content(update, context, user):
    d = user.data.get_deal(user.deal.deal_id)
...
def finish_deal(update, context, user):
    message = Txt.APPROVE_DEAL_FINISH_TEXT.format(user.deal.deal_id)
...
def warehouse_return_deal(update, context, user):
    message = Txt.APPROVE_WAREHOUSE_RETURN_TEXT.format(user.deal.deal_id)
...
def approve_warehouse_return_deal(update, context, user):
    message = Txt.WAREHOUSE_RETURN_TEXT.format(user.deal.deal_id)
