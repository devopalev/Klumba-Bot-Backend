...
import source.handlers.Approvement.FestiveCBQ as FestiveCBQ
...
import source.handlers.Approvement.TextSnippets as Txt
from source.handlers.Approvement.State import State
from source.Users import BaseUser
import source.handlers.Approvement.BitrixHandlers as BH
import source.utils.Utils as Utils
import source.Commands as Cmd
import source.config as cfg
import source.bitrix.BitrixWorker as BW
import source.bitrix.BitrixFieldsAliases as BFA
import source.bitrix.BitrixFieldMappings as BFM


@TgCommons.pre_handler(False)
...
def festive_comment(update: Update, context, user: BaseUser):
    ...
    if unapproved_chat_id:
        unapproved_chat = Chat(bot=context._bot, id=unapproved_chat_id, type=Chat.SUPERGROUP)
        keyboard = [
        ...
        if user.festive_data.photo_urls:
            ...
            context._bot.send_media_group(chat_id=unapproved_chat_id, media=media_list)
...
def festive_reapprove(update: Update, context, user: BaseUser):
    ...
    HttpJobs.send_festive_deal_message(context._bot, deal_id, deal_stage, deal_order,
